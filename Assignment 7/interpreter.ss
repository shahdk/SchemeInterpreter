    ...
    (if (null? arg) (eopl:error 'apply-primitive-proc "Missing procedure body")
        (case id
          ...
          [(sub1) (cond
                    ...
                    [else (sub1 (car arg))])]
          [(++) (cond
                    [(or (not (number? (car arg))) (null? (car arg))) (eopl:error 'apply-primitive-proc "~s Not a number in #<primitive procedure ++>" (car arg))]
                    [(not (null? (cdr arg))) (eopl:error 'apply-primitive-proc "Incorrect argument count ~s in #<primitive procedure add1>" ++)]
                    [else (add1 (car arg))])]
          [(zero?) (cond
