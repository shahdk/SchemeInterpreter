    ...
    (cases expression exp
      ...
      [or-exp (body)
        (if (null? body)
            ...
            (if (null? (cdr body))
                (expand-syntax (car body))
                (expand-syntax (let-exp (list 'tempVal) (list (car body))
                                 (if-exp (var-exp 'tempVal) 
                                   (var-exp 'tempVal)
                                   (or-exp (cdr body)))))))]
      [cond-exp (conditions) 
        (cond 
...
(define eval-expression
  (lambda (exp env)
    (cases expression exp
      ...
      [letrec-exp (symbols values body) 
        (let* ([vals (map (lambda (v) (eval-expression v env)) values)]
               [letrec-env (extend-env-recur symbols vals env)])
		  (eval-expression body letrec-env))]
    ;  [let-exp (symbols values body)
    ;    (eval-expression (app-exp (lambda-exp symbols body) values) env)]
    ;  [let*-exp (symbols values body) 
    ;    (eval-expression (expand-syntax (cons 'let*-exp (cons symbols (list values body)))) env)]
      [begin-exp (expressions)
...
;        (cond
;          [(null? body) #t]
;          [(null? (cdr body)) (eval-expression (car body) env)]
;          [else (letrec ([loop (lambda (body)
;                                 (cond
;                                   [(null? (cdr body)) (eval-expression (car body) env)]
;                                   [(eval-expression (car body) env) (loop (cdr body))]
;                                   [else #f]))])
;                  (loop body))])]
;      [or-exp (body)
;        (cond
...
;                                 (cond
;                                   [(null? (cdr body)) (eval-expression (car body) env)]
;                                   [(not (eval-expression (car body) env)) (loop (cdr body))]
;                                   [else (eval-expression (car body) env)]))])
;                  (loop body))])]
      [define-exp (name body)
        (if (null? env)
            (extend-global-env name (eval-expression body (empty-env)))
			(add-to-env name (eval-expression body env) env))]
			;(extend-env name (eval-expression body env) env))]
      [cond-exp (conditions)
        (eval-expression (expand-syntax (cons 'cond-exp (list conditions))) env)]
      [null-exp () (void)]
      [else
