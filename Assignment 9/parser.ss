...
(define initial-parse
  (lambda (datum)
    (cond
      ...
      [(pair? datum)
			(if (list? datum)
					(app-exp (initial-parse (car datum))
                     (map initial-parse (cdr datum)))
					 (lit-exp datum))]
       ...
       (case (car datum)
         ...
         ['let
          (if (list? (cadr datum))
              ...
              ;  (begin (find-replace (map parse-expression (map list symbols)) body symbols)
         ...
         ['cond
          (cond
            [(null? (cdr datum)) (eopl:error 'parse-expression "cond expr: missing clauses in ~s" datum)]
            [(not (valid-clause? (cdr datum))) (eopl:error 'parse-expression "cond expr: incorrect clauses in ~s" datum)]
            [else (cond-exp (map cd-lambda (cdr datum)))])]
         ['case
          (cond
            [(null? (cdr datum)) (eopl:error 'parse-expression "case expr: missing key and clauses in ~s" datum)]
            [(null? (cddr datum)) (eopl:error 'parse-expression "case expr: missing clauses in ~s" datum)]
            [(not (valid-clause? (cddr datum))) (eopl:error 'parse-expression "case expr: incorrect clauses in ~s" datum)]
         ...
         ['define
          (cond
            [(null? (cadr datum)) (eopl:error 'parse-expression "define expr: missing variable to define in ~s" datum)]
            [(null? (caddr datum)) (eopl:error 'parse-expression "define expr: missing body in ~s" datum)]
...
(define find-replace
  (lambda (find-ele body syms)
    (cond
      [(null? find-ele)]
      [else (begin (map (lambda (body) (if (and (not (equal? (caar find-ele) 'var-exp)) (not (equal? (caar find-ele) 'lit-exp)) (not (equal? (caar find-ele) 'app-exp)))
                                           ((substitute-all-m (caar find-ele) (cons 'var-exp (list (car syms)))) body))) body)
                     (find-replace (cdr find-ele) body (cdr syms)))])))
  ...
  (lambda (old new)
      (lambda (ls)
        (letrec ([sub-all-m (lambda (old new ls)
                                     (if (atom? ls)
                                         (if (equal? old ls) (set-car! (list ls) new))
                                         (if (equal? old (car ls))
                                             (let ([temp (cdr ls)])
                                               (begin (set-car! ls 'app-exp) (set-cdr! ls (list new)) (set-cdr! (cdr ls) temp) (sub-all-m old new (cddr ls)))) 
                                             (begin (sub-all-m old new (car ls)) (sub-all-m old new (cdr ls))))))])
