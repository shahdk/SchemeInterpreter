...
(define eval-one-exp
  (lambda (exp)
    (let* ([parse-tree (initial-parse exp)]
           [result (eval-expression parse-tree (empty-env))])
      result)))

(define eval-one-exp-actual
  (lambda (exp)
    ...
    (cases expression exp
      ...
      [app-exp (operator operand)
        (let* ([procedure (eval-expression operator env)])
              (if (equal? procedure '#f)
                (eval-one-exp-actual (unparse-expression exp))
                (let ([arg (map (lambda (operand) (eval-expression operand env)) operand)])
        ...
        (case id
          ...
          [(map) (cond
                   ...
                   [else (map (lambda (args)
                                (apply-proc (car arg) args  '())) (index-recur (cdr arg)))])]
          [(apply) (cond
		  ...
		  [(load) (let ([p (open-input-file (car arg))])
					(let f ([x (read p)])
						(if (eof-object? x)
						(begin (close-port p) '())
						(begin (eval-one-exp-actual x) (f (read p))))))]
...
(define primitive-proc
  (list '+ '- '* '/ 'add1 'sub1 'max
    ...
    'atom? 'length 'list->vector 'list? 'load
